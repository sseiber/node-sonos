...
/**
 * Plays Pandora based on station id
 * @param  {String}   stationId  Pandora radio station id
 * @param  {Function} callback (err, playing)
 */
Sonos.prototype.playPandoraRadio = function (stationId, stationTitle, callback) {
  debug('Sonos.playTuneinRadio(%j, %j, %j)', stationId, stationTitle, callback)
  var self = this
  if (!stationId || !stationTitle) {
    return callback(new Error('stationId and stationTitle are required'))
  ...
  //var options = optionsFromTuneinUri(stationId, stationTitle)
  var metadata = '<DIDL-Lite xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:upnp="urn:schemas-upnp-org:metadata-1-0/upnp/" xmlns:r="urn:schemas-rinconnetworks-com:metadata-1-0/" xmlns="urn:schemas-upnp-org:metadata-1-0/DIDL-Lite/"><item id="OOOX##TUNEINSTATIONID##" parentID="0" restricted="true"><dc:title>##TUNEINSTATIONTITLE##</dc:title><upnp:class>object.item.audioItem.audioBroadcast</upnp:class><desc id="cdudn" nameSpace="urn:schemas-rinconnetworks-com:metadata-1-0/">SA_RINCON3_svclpdev@microsoft.com</desc></item></DIDL-Lite>'
  metadata = metadata.replace('##TUNEINSTATIONID##', stationId).replace('##TUNEINSTATIONTITLE##', stationTitle)
  var action = '"urn:schemas-upnp-org:service:AVTransport:1#SetAVTransportURI"'
  var body = '<u:SetAVTransportURI xmlns:u="urn:schemas-upnp-org:service:AVTransport:1"><InstanceID>0</InstanceID><CurrentURI>pndrradio:' + stationId + '?sn=4</CurrentURI><CurrentURIMetaData>' + htmlEntities(metadata) + '</CurrentURIMetaData></u:SetAVTransportURI>'
  return this.request(this.options.endpoints.transport, action, body, 'u:SetAVTransportURIResponse', function (err, data) {
    if (err) return callback(err)
    if (data[0].$['xmlns:u'] === 'urn:schemas-upnp-org:service:AVTransport:1') {
      return self.play(callback)
